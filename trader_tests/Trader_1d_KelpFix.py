...
class Kelp(Product):
    ...
    def fair_val(self):
        ...
        mm_ask_price, mm_ask_qty = min(self.order_depth.sell_orders.items(), key = lambda x: x[1]) # bffr????
        
        avg = mm_bid_price + mm_ask_price

        # incredibly stupid balancing algorithm to somewhat mitigate rounding bias if fv = x + 0.5
        if avg % 2 == 1:
            if self.timestamp % 200 == 100:
                avg += 1
            else:
                avg -= 1
