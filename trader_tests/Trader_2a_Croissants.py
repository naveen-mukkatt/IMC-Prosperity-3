...
class Product():
    ...
    def strategy(self): # OHHHHH I GET IT NOW! IT'S RUNTIME POLYMORPHISM
...
class Ink(Product):
    ...
    def fair_val(self):
        ...
        if prev_ink_bid == 0:
            prev_ink_bid = mm_bid_price
        if prev_ink_ask == 0:
            prev_ink_ask = mm_ask_price
        if prev_ink_mid == 0:
            prev_ink_mid = mid

        val = 0.9 * mid + 0.1 * prev_ink_mid
        prev_ink_bid = mm_bid_price
        prev_ink_ask = mm_ask_price
        prev_ink_mid = mid
        return val
    ...
    def strategy(self):
        ...
        if abs(mm_bid_price - prev_ink_bid) > threshold or abs(mm_ask_price - prev_ink_ask) > threshold:
            return
...
class Croissant(Product):
    ...
    def fair_val(self):
        global prev_crossiant_mid

        mm_bid_price, mm_bid_qty = max(self.order_depth.buy_orders.items(), key = lambda x: x[0])
        mm_ask_price, mm_ask_qty = min(self.order_depth.sell_orders.items(), key = lambda x: x[0])
        
        mid = (mm_bid_price + mm_ask_price)/2
        if prev_crossiant_mid == 0:
            prev_crossiant_mid = mid

        self.state.traderData = self.state.traderData + " " + str(mid)
        
        val = mid * 0.5 + prev_crossiant_mid * 0.5
        prev_crossiant_mid = mid
        return val
...
class Trader:
    def executor(self, product: str, state: TradingState): # add product to execute set of strategies
        ...
        '''
            ...
            croissant = Croissant(product, 250, state)
            return croissant.execute()
        '''
